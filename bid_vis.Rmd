---
title: "Sociedades desiguales y un pronóstico desalentador"
output: 
...
### Desfile de Pen's  
```{r}
load("C:/Users/ALVARO/Documents/bid.RData")

ggplot(bdpen,aes(qq,index))+geom_point(aes(color=pais))+xlab("Centiles (q)")+ylab("suma(ipcm_q)/max(suma(ipcm_q))")+transition_time(year)+labs(title = 'Año: {frame_time}') 
...
### Percentil 95 vs 5. 1999 - 2030
```{r}
p2<-ggplot(mdiff,aes(year,round(index,2)))+geom_point(aes(color=pais))+facet_wrap(~pais)+ylim(0,200)
ggplotly(p2)
```
...
#ggplotly(p2)
p2<-ggplot(mgini,aes(year,index))+geom_point(aes(color=pais))+ylim(0.25,0.75)
ggplotly(p2)
```
